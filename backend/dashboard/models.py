...
class Dish(models.Model):
    ...
    def save(self, *args, **kwargs):
        ...
        if any(w in nl for w in ['aloo','potato','alu ']):  self.is_aloo = True
        if any(w in nl for w in ['dal','daal','kadhi','rajma','chana','moong',
                                   'arhar','toor','urad','masoor','sambhar',
                                   'sambar','sambher','choley','chooley']): self.is_dal = True
        super().save(*args, **kwargs)

    def to_dict(self):
        return {'id':self.pk,'dish_name':self.dish_name,'meal_type':self.meal_type,
                'ingredients':self.ingredients or '','brand':self.brand,
                'is_dal':self.is_dal,'is_star':self.is_star,'is_aloo':self.is_aloo,
